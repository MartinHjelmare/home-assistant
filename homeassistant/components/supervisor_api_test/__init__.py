...
class SupervisorServices:
    ...
    async def async_start_addon(self, call):
        """Start addon."""
        slug = call.data["slug"]
        repository = call.data["repository"]
        addon_id = f"{repository}_{slug}"
        await self._hassio.async_start_addon(addon_id)
        _LOGGER.warning("Started add-on: %s", addon_id)

    async def async_stop_addon(self, call):
        """Stop addon."""
        slug = call.data["slug"]
        repository = call.data["repository"]
        addon_id = f"{repository}_{slug}"
        await self._hassio.async_stop_addon(addon_id)
        _LOGGER.warning("Stopped add-on: %s", addon_id)
